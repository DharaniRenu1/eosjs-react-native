...
export const generateRandomPrivKeyP = () =>
  ...
  export const generateSeedPrivKey = (mnem) =>
  new Promise((resolve, reject) => {
    privateKey = ecc.PrivateKey.fromSeed(mnem).toString()
...
export const postMurmur = async (accountName, post, visibility = '') => {
  let transaction = await eos.transaction('murmurdappco', contract => {
    contract.murmur(accountName, post, visibility);
  }, config)

  console.log("postMurmur", transaction);
}

export const murmurSignup = async (accountName, publicKey = '') => {
